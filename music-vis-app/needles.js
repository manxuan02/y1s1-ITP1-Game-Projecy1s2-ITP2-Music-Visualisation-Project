	...
	this.draw = function() {
		...
		for (var i = 0; i < this.plotsDown; i++) {
			for (var j = 0; j < this.plotsAcross; j++) {
				...
				var x = this.pad+j*this.plotWidth; //x-axis of top left
				var y = this.pad+i*this.plotHeight; //y-axis of top left
				var w = this.plotWidth-this.pad; //width
				var h = this.plotHeight-this.pad; //height
				//draw a rectangle at that location and size
				rect(x,y,w,h);

				//add on the ticks
				this.ticks(x+w/2,y+h,this.frequencyBins[currentBin]); //need to put according to the arguments `this.ticks(centreX, bottomY, freqLabel)`

				var energy = fourier.getEnergy(this.frequencyBins[currentBin]);

				//add the needle
				this.needle(energy,x+w/2,y+h); //need to put according to the arguments 'this.needle(energy,centreX,bottomY)'
